...
set_post_thumbnail_size(300,140, true );
//支持文章格式
...
function ria_comment( $comment, $args, $depth ) {
	$GLOBALS['comment'] = $comment;
	switch ( $comment->comment_type ) :
		case '' :
	?>
	<li <?php comment_class('comment-item J_CommentListItem'); ?> id="li-comment-<?php comment_ID(); ?>">
		<div class="comment-single" id="comment-<?php comment_ID(); ?>">
            <figure class="member member-author comment-author vcard">
                <?php echo get_avatar( $comment, 50 ); ?>
                <figcaption><b><?php echo get_comment_author_link(); ?></b></figcaption>
            </figure>
            <div class="comment-content comment-body"><?php comment_text(); ?></div>
            <time datetime="<?php echo get_comment_date().' '.get_comment_time();?>">
                <?php echo get_comment_date('Y-m-d').' '.get_comment_time();?>
            </time>
	    </div>
